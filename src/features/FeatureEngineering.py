...
from src.features.contracts import CATE_COLS, ID_COLS, TARGET


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Tạo feature cho fraud detection và split dữ liệu theo thời gian."
    )

    parser.add_argument("--raw-input", type=str, default="data/raw/transactions.csv")
    parser.add_argument("--valid-start", type=str, default="2024-01-01")
    parser.add_argument("--test-start", type=str, default="2024-04-01")
...
def basic_prepare(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df = df.drop(columns=["fraud_pattern", *ID_COLS], errors="ignore")
...
def time_split_by_date(
    ...
    valid_cut = pd.Timestamp(valid_start)
    test_cut = pd.Timestamp(test_start)

    if valid_cut >= test_cut:
        raise ValueError("--valid-start phải đứng trước --test-start")

    train_df = df[df["timestamp"] < valid_cut].copy()
    valid_df = df[(df["timestamp"] >= valid_cut) & (df["timestamp"] < test_cut)].copy()
    test_df = df[df["timestamp"] >= test_cut].copy()

    if min(len(train_df), len(valid_df), len(test_df)) == 0:
        raise ValueError("Split theo ngày tạo ra tập rỗng, cần kiểm tra lại mốc thời gian.")
...
def add_features(df: pd.DataFrame, params: dict) -> tuple[pd.DataFrame, list[str]]:
    ...
    # Các ngưỡng được fit từ train, nên validation/test không nhìn thấy tương lai.
    if "amount" in df.columns:
...
def prepare_log_branch(
    ...
    # Nhánh tuyến tính cần dữ liệu đã mở rộng one-hot và scale về cùng thang.
...
def prepare_tree_branch(
    ...
    # Nhánh cây giữ category gọn hơn, chỉ mã hóa thành số nguyên.
    train_tree, encoders = label_encode(train_tree, list(CATE_COLS))
    tree_columns = train_tree.columns.tolist()

    valid_tree, _ = label_encode(valid_tree, list(CATE_COLS), encoders=encoders)
    valid_tree = valid_tree.reindex(columns=tree_columns, fill_value=0)

    test_tree, _ = label_encode(test_tree, list(CATE_COLS), encoders=encoders)
...
def main() -> None:
    ...
    raw_df = basic_prepare(load_table(args.raw_input))
    train_df, valid_df, test = time_split_by_date(
        raw_df,
    ...
    params["valid_start"] = args.valid_start
    params["test_start"] = args.test_start
